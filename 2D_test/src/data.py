...
class PIPDatasetGLS(torch.utils.data.Dataset):
    ...
    def gen_mas1_params(self, n, wr=None):
        """
    ...
    def __getitem__(self, _, wr=None):
        """
        Generate an input
        """
        ...
        wr, lws, ms, ss, ps = self.gen_mas1_params(n, wr=wr)
        self.wr = wr
...
class PIPDatasetGLS2D(torch.utils.data.Dataset):
    """
    Generating PIP dataset with GLS broadening
    """

    def __init__(self, data_pars_x, data_pars_y, rot_range=[], mult=5.):
        """
        Initialize the class
        """

        self.dataset_x = PIPDatasetGLS(**data_pars_x)
        self.dataset_y = PIPDatasetGLS(**data_pars_y)
    ...
    def __getitem__(self, _):
        ...
        brd_specs_x, _, iso_x = self.dataset_x.__getitem__(0)
        brd_specs_y, _, iso_y = self.dataset_x.__getitem__(0, wr=self.dataset_x.wr)

        iso = torch.einsum("bj,bi->bij", (iso_x, iso_y))

        brd_specs = torch.einsum("abj,abi->abij", (brd_specs_x, brd_specs_y)) * self.mult

        # Rotate spectra
        if len(self.rot_range) > 0:

            irot = np.random.randint(len(self.rot_range))

            if isinstance(self.rot_range[0], list):
                r = self.rot_range[irot]
        ...
        return brd_specs_x, brd_specs_y, brd_specs, iso_x, iso_y, iso